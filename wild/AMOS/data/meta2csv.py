...
class ProgressBar(object):
    ...
    def Start(self, what=''):
        ...
        Make sure not to print anything until after calling
...
class DicomSeries(object):
    ...
    def description(self):
        ...
        and ImageComments.
        """
        # TODO: filter infomation and only leave those useful
        info = self.info
        ...
        # Give patient name (notice: if there's no name,the string can't be concatenated to a null variable)
        if info.PatientName != '':
    ...
    def _finish(self):
        ...
        for index in range(len(L)):
            ...
            # ImageOrientationPatient? Do you mean ImagePositionPatient.
            # Q & A: I guess you mean the normal has to adapt to different scales of the 3D image sizes. e.g.
            # someone's organ is larger or smaller than others.
...
def meta2csv(item):
    ...
    data_list = []
    if os.path.isdir(item):
        all_series = read_files(item, False, True)

    for series in all_series:
        check_id = os.path.basename(item)
        seris_id = series.info.SeriesDescription
        meta_dict = {"check_id": check_id,
                        "seris_id": seris_id,
                        "spacing": series.spacing,
                        "shape": series._shape,}
        for ele in series.info:
            try:
...
if __name__ == '__main__':
    ...
    with Pool(8) as p:
        r = itertools.chain(*tqdm(p.map(meta2csv, dir_list), total=len(dir_list)))
