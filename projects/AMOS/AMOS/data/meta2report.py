...
csvs = glob.glob(data_dir+'/*/*.csv', recursive=True)
overall_data_df = pd.concat([pd.read_csv(f) for f in csvs ])
overall_data_df.to_csv( os.path.join(data_dir, "overall_data.csv"), index=False, encoding='utf-8')

report_df = pd.read_csv(os.path.join(data_dir, "overall_data.csv"))
meta_df = pd.concat([pd.read_excel(dr) for dr in series_meta_dirs])
# 提取之前已标注的病人病历号
pre_df = pd.concat([pd.concat(pd.read_excel(dr, usecols=['病历号', 'ann_flag'], sheet_name=None), ignore_index=True) for dr in pre_dirs])
pre_df = pre_df[pre_df['ann_flag'] == 1]
...
for item in niigzs:
    ...
    meta_res = meta_df.loc[(meta_df["check_id"] == _check_id) & (meta_df["Study Instance UID"] == _study_id) & (meta_df["Series Instance UID"] == _series_id)]
    if len(meta_res) ==1:
...
over_res.to_excel(os.path.join(root_dir, "secondround_ct_data_meta_202101.xlsx"), encoding='utf-8', index=False)